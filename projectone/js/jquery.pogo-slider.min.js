	...
	e.prototype = {
		...
		_onSlideStart: function(a) {
			if (this.slides[a].$element.css("z-index", 1), this.settings.autoplay && (this.slideStartTime = new Date, this.slideTimeRemaining = this.slides[a].totalSlideDuration, this.settings.displayProgess && !this.paused && this.slides[a].$element.find(".pogoSlider-progressBar-duration").css("width", "0").animate({
					width: "100%"
				}, this.slideTimeRemaining, "linear")), this.slides[a].children.length > 0 && this._slideElementsTransitionIn(a), this.paused)
				for (var b = 0; b < this.slides[a].children.length; b++) this.slides[a].children[b].$element.precss("animation-play-state", "paused");
		...
		_createSlideSlices: function(a, b, c) {
			var d, e = c * b,
				...
				k = j.attr("style");
			if (d = this.paused ? this.slides[a].totalSlideDuration - this.slideTimeRemaining : this.slides[a].totalSlideDuration - (this.slideTimeRemaining - (new Date - this.slideStartTime)), a === this.prevSlideIndex && this.slides[a].children.length > 0 && d < this.slides[a].elementTransitionDuration)
		...
		slideTransitions: {
			...
			foldRight: function(a, b) {
				...
				var g = c.slides[b].$element.find(".pogoSlider-slide-slice"),
					...
					i = g.eq(0),
					j = g.eq(1);
				return d.$element.prepend(h.detach()), e.$element.prepend(i.detach()), h.addClass("pogoSlider-animation-foldInRight").precss("animation-duration", d.duration + "ms"), j.addClass("pogoSlider-animation-foldOutRight").precss("animation-duration", d.duration + "ms"),
					function() {
			...
			foldLeft: function(a, b) {
				...
				var g = c.slides[b].$element.find(".pogoSlider-slide-slice"),
					...
					i = g.eq(0),
					j = g.eq(1);
				return d.$element.append(h.detach()), e.$element.append(j.detach()), h.addClass("pogoSlider-animation-foldInLeft").precss("animation-duration", d.duration + "ms"), i.addClass("pogoSlider-animation-foldOutLeft").precss("animation-duration", d.duration + "ms"),
					function() {
